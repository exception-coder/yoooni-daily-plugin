...
.SYNOPSIS
  ...
  与 skills\yoooni-install-team-tools\install-team-tools.ps1 的区别：
    install-team-tools.ps1 跑在【本体插件内部】，装不了本体自己，也不注册定时更新；
    本脚本是【零依赖引导】，多做两件它做不了的事：
      (1) 连「本体插件 yoooni-daily-plugin」自身一并克隆 + 安装；
      (2) 自动注册定时自动更新任务（默认每 1 小时，内容近期频繁更新）。
...
.PARAMETER EveryHours   定时更新周期（小时），默认 1；设 0 跳过注册定时任务。
...
Write-Host "定时更新  : 每 $EveryHours 小时"
...
if (HasGitDir $selfDir) { Write-Host '  = 仓库已存在，跳过克隆（更新交给定时任务/更新 skill）' -ForegroundColor DarkGray }
...
Write-Host "后续保持最新：定时任务每 $EveryHours 小时自动跑；开 Claude Code 会话也会触发 SessionStart 刷新。" -ForegroundColor Cyan
Write-Host '手动更新：『更新公司套件』skill(yoooni-update-team-tools) 或 scripts\update-team-tools.ps1。' -ForegroundColor Cyan
Write-Host '改更新周期：scripts\register-autoupdate-task.ps1 -EveryHours <小时>（内容稳定后建议改回 2~4）。' -ForegroundColor DarkCyan
Write-Host '==============================================' -ForegroundColor Cyan
