...
description: 仅用于 Yoooni 新同事入职并通过 SVN 首次拉取公司项目文档。只有用户明确提到“入职/新同事”并要求搭建环境，或明确提到“SVN checkout/拉取 Yoooni 公司项目文档”时使用。不要因泛化的“初始化环境”“初始化项目”“初始化项目文档”“初始化知识图谱”“生成知识图谱”而触发；这些请求不属于入职流程，也不得检查 SVN 或创建 svn-auth.json。
---
...
- 在已经明确讨论 Yoooni 入职流程的上下文中询问“第一步做什么”

## 禁止误触发

以下表达必须排除本 skill：

- “初始化当前项目文档和知识图谱”“生成知识图谱” → 交给 `team-standards:init-project-docs`
- “同步/更新项目文档或知识图谱” → 交给 `team-standards:project-docs-update`
- 单独说“初始化环境”“初始化项目”“拉一下文档”，但没有入职、Yoooni 公司文档或 SVN 上下文

触发证据不足时先询问用户要初始化的是“入职 SVN 文档”还是“当前代码项目知识图谱”，不得先运行 SVN、读取凭据或创建配置文件。
